...
class WikipediaSuite extends FunSuite with BeforeAndAfterAll {
  ...
  // test I added
  test("'occurrencesOfLang' with multiple elements") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val rdd = sc.parallelize(Seq(WikipediaArticle("title", "Java Jakarta"), WikipediaArticle("title", "People who love Java")))
    val res = (occurrencesOfLang("Java", rdd) == 2)
    assert(res, "occurrencesOfLang given (specific) RDD with one element should equal to 1")
  }

  // test I added but now won't pass because I changed the function's scope
 /* test("'getLangs'") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val res = (getLangs(List("Java", "Scala", "C++"), WikipediaArticle("title", "Java Scala")) == List("Java" -> WikipediaArticle("title", "Java Scala"), "Scala" -> WikipediaArticle("title", "Java Scala")))
    assert(res, "getLangs is correct")
  }*/
