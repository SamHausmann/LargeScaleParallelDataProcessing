...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    ...
    val restOfColumns = columnNames
    .drop(1)
    .map(x => StructField(x, DoubleType, false))

    StructType(firstColumn::restOfColumns)
  }
  ...
  def row(line: List[String]): Row = {
    Row
    .fromSeq(line.map{ x => if (line.indexOf(x) == 0) x 
      else x.toDouble})
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    .filter(x => x.startsWith("t01") || x.startsWith("t03")  || x.startsWith("t11") || x.startsWith("t1801") || x.startsWith("t1803"))

    val workingActivities = columnNames
    .filter(x => x.startsWith("t05") || x.startsWith("t1805"))

    val nonOthers = primaryNeeds::workingActivities

    val acts = columnNames
    .filter(x => x.startsWith("t02") || x.startsWith("t04")  || x.startsWith("t06") || x.startsWith("t07") || x.startsWith("t08") || x.startsWith("t09") || x.startsWith("t10") || x.startsWith("t12") || x.startsWith("t13") || x.startsWith("t14") || x.startsWith("t15") || x.startsWith("t16") || (x.startsWith("t18") && !x.startsWith("t1801") && !x.startsWith("t1803")))
    
    val otherActivities = acts
    .diff(nonOthers)

    (primaryNeeds.map(y => new Column(y)), workingActivities.map(y => new Column(y)), otherActivities.map(y => new Column(y)))
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    .map {case ((a, b, c), d, e, f) => TimeUsageRow(a, b, c, d, e, f)}
